...
import { getTransactionbyHost } from '../api/api'; 
...
const TransactionHistory = ({ user }) => {
  ...
  const fetchTransactions = async () => {
    const transaction = await getTransactionbyHost();
    const transactionHistory = transaction.data?.transactions || [];
  ...
  const columns = [
    ...
    { title: 'Hotel', key: 'hotel', render: (record) => record.roomId?.roomType || 'N/A' },
      ...
      <Table 
        ...
        onRow={(record) => ({
          onDoubleClick: () => {
            ...
            navigate(`/host/transaction-detail/${record.transactionId}`);
          }
        })}        
      />
    </div>
